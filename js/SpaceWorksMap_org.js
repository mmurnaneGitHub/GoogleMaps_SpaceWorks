...
//for Itinerary directions
  ...
  //mobile device check
    ...
    //End Add Itinerary button as a control------------------------

    //Add HELP button as a control------------------------
      // Create a div to hold the control.
      var controlDiv = document.createElement('DIV');
      ...
      //Add HELP button to map legend
      gmap.controls[google.maps.ControlPosition.LEFT_TOP].push(controlDiv);
      //Add events
      google.maps.event.addDomListener(controlUI, 'click', function() {
        dijit.byId('mapWelcome').show();  //Open help dialog
      });
      google.maps.event.addDomListener(controlUI, 'mouseover', function() {
      });

    //End HELP button as a control------------------------
...
//for Itinerary list
          ...
          load: function(jsonData) {
              ...
              //Studio thumbnail summary - use array of studio markers (studioMarker) for click event
                  ...
                  var sum_content =  "<div style=\"text-align:center;\"><b>" +  jsonData.items[i].Project_Name + "</b>";
                      ...
                      detail_content += "<br><a href=\"https://" + jsonData.items[i].Website + "\" target='_blank'>" + jsonData.items[i].Website + "</a><br>&nbsp;";
                      detail_content += "</div>";
                      ...
                      detail_content += "<a href=\"https://maps.google.com/?output=classic&cbll=" + jsonData.items[i].Latitude +"," + jsonData.items[i].Longitude +"&cbp=13,0,,,&layer=c&z=17\" target='_blank'>Street View</a>";
              ...
              //Add markerCluster symbols to map now that all indiviidualmarkers have been added
              var markerCluster = new MarkerClusterer(gmap, markers, {maxZoom: 17});
              //could change the mouse-over for the cluster title (click to zoom-in)
              //markerCluster.setStyles(styles:[{"title":"test"}]);
              //alert(dojo.toJson(markerCluster.getStyles()));
              //var markerStyle = markerCluster.getStyles();
              //markerStyle.push({"title":"test"});
              //alert(dojo.toJson(markerStyle));
              //markerCluster.setStyles(styles: markerStyle);
 ...
 function showIconLegend(icon) {
  ...
  function addMarker(Latitude, Longitude, sum, info, title, imageIcon) {
    ...
    //Add marker to MarkerClusterer array
    markers.push(marker);

    //Add marker events
    google.maps.event.addListener(marker, 'mouseover', function() {
      //show popup, highlight marker

      //Recenter map if marker is outside current map extent
      if (!(gmap.getBounds().contains(marker.getPosition()))){
        gmap.setCenter(location);          //use maker lat/long to recenter map
      }
