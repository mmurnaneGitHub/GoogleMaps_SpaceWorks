...
//for Itinerary directions
  ...
  //=== Set marker attributes ===
    ...
    var imageSatSun = new google.maps.MarkerImage('images/mapIcons/SatSun.png',
        new google.maps.Size(12, 20),
        new google.maps.Point(0,0),
        new google.maps.Point(0, 20));
    var imageSun = new google.maps.MarkerImage('images/mapIcons/Sun.png',
        new google.maps.Size(12, 20),
        new google.maps.Point(0,0),
        new google.maps.Point(0, 20));
    var imageSat = new google.maps.MarkerImage('images/mapIcons/Sat.png',
        new google.maps.Size(12, 20),
        new google.maps.Point(0,0),
        new google.maps.Point(0, 20));
  ...
  //mobile device check
    ...
    //Add LOGO as a control------------------------
      ...
      google.maps.event.addDomListener(controlUI, 'click', function() {
        //alert("here");
        window.open('http://www.artatworktacoma.com/');
      });
    ...
    //Add Legend as a control------------------------
      ...
      // Set CSS for the control border
      var controlUI = document.createElement('DIV');
      /*controlUI.style.backgroundColor = '#00848E';*/
      controlUI.style.backgroundColor = '#7A7D81';
      controlUI.style.borderStyle = 'solid';
      controlUI.style.borderWidth = '1px';
      controlUI.style.borderColor = '#b2b2b2';
      controlUI.style.textAlign = "center";
    ...
    //Add Legend as a control------------------------
      ...
      // Set CSS for the control border
      var controlUI = document.createElement('DIV');
      /*controlUI.style.backgroundColor = '#C0004E';*/
      controlUI.style.backgroundColor = '#E15F55';
      controlUI.style.borderStyle = 'solid';
      controlUI.style.borderWidth = '1px';
      controlUI.style.borderColor = '#b2b2b2';
      controlUI.style.textAlign = "center";
    ...
    //Add Legend as a control------------------------
      ...
      // Set CSS for the control border
      var controlUI = document.createElement('DIV');
      /*controlUI.style.backgroundColor = '#F95602';*/
      controlUI.style.backgroundColor = '#6BD9DE';
      controlUI.style.borderStyle = 'solid';
      controlUI.style.borderWidth = '1px';
      controlUI.style.borderColor = '#b2b2b2';
      controlUI.style.textAlign = "center";
    ...
    //Add Zoom Home image as a control------------------------
      ...
      controlUI.style.borderStyle = 'solid';
      controlUI.style.borderWidth = '1px';
      controlUI.style.borderColor = '#7B7B7B';
      controlUI.style.cursor = 'pointer';
      
      //Add logo image
      var myLogo = document.createElement("img");
          myLogo.src = "images/HomeYellow.png";
          ...
          load: function(jsonData) {
              ...
              //Studio thumbnail summary - use array of studio markers (studioMarker) for click event
                  ...
                  if (parseInt(jsonData.items[i].StudioNumber) > previousStudioNumber) {
                    //alert(jsonData.items[i].StudioNumber + ":" + previousStudioNumber);
                    if (previousStudioNumber != 0) {//add markers for PREVIOUS Studio (just once, multiple artists)
                      //Set marker color
                      var imageIcon = imageSatSun;  //default Saturday & Sunday
                      if (previousStudioNumber != 30) {    //exception for Studio 30 , two artist on different days
                        if (jsonData.items[i-1].DayPreference=="Saturday") {
                          imageIcon = imageSat;
                        } else if (jsonData.items[i-1].DayPreference=="Sunday") {
                          imageIcon = imageSun;
                        }
                      }
                      //Add marker to map - just one per studio
                      addMarker(jsonData.items[i-1].Latitude, jsonData.items[i-1].Longitude, sum_content, detail_content, title_content, imageIcon);
                ...
                //DETAILED DESCRIPTIONS---------------------------------------
                  detail_content += "<div style='clear:both;float:left;width: 375px;'>";  //start address header
                      ...
                      if (jsonData.items[i].StudioNumber > previousStudioNumber) {
                        ...
                        detail_content += "<br><a href=\"http://maps.google.com/maps?daddr=" + iAddress + ", Tacoma, WA\" target='_blank' title='Get driving directions.'>Get Directions</a>";
  
                        //Add to Calendar 
                        var theDay=3; //default Saturday & Sunday
                        if (previousStudioNumber != 29) {    //exception for Studio 30 , two artist on different days - make SAT-SUN
                        ...
                        detail_content += "&nbsp;|&nbsp;<a href=\"http://wspdsmap.ci.tacoma.wa.us/website/HistoricMap/scripts/calendar/Calendar.asp?Day=" + theDay + "&Studio=" + jsonData.items[i].StudioNumber + "&Address=" + iAddress + "&Name=" + jsonData.items[i].FirstName + " " + jsonData.items[i].LastName + "\" target='_blank' title='Open file to add appointment to your calendar.'>Add to Calendar</a>";

                        //Zoom to Studio
                        detail_content += "&nbsp;|&nbsp;<a href='javascript:go2studio(" + jsonData.items[i].Latitude + ", " + jsonData.items[i].Longitude + ")' title='Close this window and zoom map to Studio.'>Zoom Map</a>";

                        //Street View
                        detail_content += "&nbsp;|&nbsp;<a href=\"http://maps.google.com/?output=classic&cbll=" + jsonData.items[i].Latitude +"," + jsonData.items[i].Longitude +"&cbp=13,0,,,&layer=c&z=17\" target='_blank'  title='View Studio from nearest street.'>Street View</a><br>";
                          ...
                          detail_content += "<a href=\"javascript:myLightbox('images/studios/" + jsonData.items[i].PhotoFileName.replace(/'/g,"\\'") +".jpg','" + jsonData.items[i].FirstName + " " + jsonData.items[i].LastName.replace(/'/g,"\\'") + "')\"><img style ='float:left;margin:2px 5px 5px 5px;border:solid 1px #999;padding:2px' src=\"images/studios/" + jsonData.items[i].PhotoFileName +"T.jpg\" title='Click to enlarge photo' height='100px' width='75px'></a>";
                            ...
                            detail_content += "<div style='float:left;width:35px;'>";
                              ...
                              if (jsonData.items[i].FundedbyTAC == "Yes") {
                               detail_content += "<a href=\"javascript:showIconLegend('iconTACfunded');\"><img src=\"images/Logo/TACfunded.jpg\" title=\"artist funded by Tacoma Arts Commission in 2014\" style=\"width:20px;height:20px;margin:2px 5px 5px 0px;border:solid 1px #999;padding:2px\"></a>"; 
                               //start here!!!!! 
                              }
                      ...
                      detail_content += "<div style='width: 240px;float:left;'>";
                          ...
                          //remove any "" with "
                          //detail_content += jsonData.items[i].ArtistStatement + "<br>";
                          ...
                          if (jsonData.items[i].AccessDirection != "") {
                            detail_content += "<i>Additional Directions: " + jsonData.items[i].AccessDirection + "</i><br>";
                          }
                          //Phone
  ...
  function addMarker(Latitude, Longitude, sum, info, title, imageIcon) {
    ...
    google.maps.event.addListener(marker, 'mouseover', function() {
      ...
      //map tip - summary window
        ...
        //dijit.popup.open({ popup: dialog, x: containerPixel.x, y: containerPixel.y }, ["BL","TL","BR","TR"], {x: 15, y: 0});
